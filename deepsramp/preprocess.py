''' This file contains some functions for preprocessing the raw data '''
...
def get_mature_splice(gtf, seqs, biotype='protein_coding', grp=['gene', 'trans'], keepmax=False):
    ''' Get mature splice junction from GTF object '''
    sdf = gtf[
        ((gtf['biotype'] == biotype) if biotype else True) &
        (gtf[2] == 'exon')
    ].groupby(grp).agg({
    ...
    if keepmax:
        sdf = sdf.reset_index().groupby(grp[0]).apply(
            lambda x: x[x.length == x.length.max()].sample(1)
        ).set_index(grp)
...
def get_mature_cds(gtf, sdf, biotype='protein_coding', grp=['gene', 'trans']):
    ''' Get mature CDS region from GTF object '''
...
def get_full_splice(sdf, seqs):
    ''' Get full splice junction from mature splice jucntion object '''
...
def get_full_cds(gtf, sdf, biotype='protein_coding', grp=['gene', 'trans']):
    ''' Get full CDS region from GTF object '''
    ndf = gtf[((gtf['biotype'] == biotype) if biotype else True) & (gtf[2] == 'CDS')].groupby(grp).agg({
        'range': lambda x: x.tolist(),
    })
    ndf['range'] = ndf.apply(lambda x: (min(i[0] for i in x.range), max(i[1] for i in x.range)), axis=1)
...
def get_genome_pos(pos, sdf, mode='mature'):
    ''' Get the genome position of the transcript position with through the transcript row in GTF object '''
    if mode == 'full':
        if sdf[6] == '+':
            return pos + sdf['min']
        else:
...
def get_mature_pos(k, sdf):
    ''' Get the mature transcript position of the genome position with through the transcript row in GTF object '''
...
def get_full_pos(k, sdf):
    ''' Get the full transcript position of the genome position with through the transcript row in GTF object '''
...
def get_trans(ch, st, pos, sdf):
    ''' Get transcripts by searching chromsome, strand and genome position in GTF object '''
...
def read_m6a_bed(file, name, ndf, pat, get_pos):
    ...
    m6adf['pos'] = m6adf.progress_apply(lambda x: [
        ...
        ] for i in get_trans(x[0], x[5], x[1], ndf).itertuples()
    ], axis=1)
...
def get_m6a_neg(sdf, pos_label='m6a_pos', pat=DRACH_PAT):
    ''' Get [pat] locations that not in [pos_label] column '''
